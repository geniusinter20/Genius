...
function PracticeTests() {
    ...
    useEffect(() => {
        setExpanded(practiceTests.map(x => {
            var id = -1;
            if (state != null && x.testExamsIDs.findIndex(q => q === state.finishedExamID) !== -1) id = x.key
            return (id)
        }).filter(w => w !== -1))
    }, [practiceTests])
    ...
    const examColumns = [
        ...
        },
        {
            ...
            render: (key, record) => {
                ...
                const ind = examsActivities.findIndex(x => x.examID === key)
                //console.log()
                var rateVal = -1;
                var count = 0;
                if (ind === -1) {
                    ...
                    expandable={{
                        ...
                        onExpand: (expanded1, record) => {
                            expanded.findIndex(x => x === record.key) === -1 ?
                                ...
                                setExpanded(expanded.filter(q => q !== record.key))
                        },
